...
# SpoolHard Scale — Release Build Script
# Builds firmware + frontend, creates SPIFFS image, and generates OTA manifest.
# Suitable for local use and CI/CD (GitHub Actions).
#
# Usage:
...
#
# Requires: node 18+, npm, platformio (pio), python3
...
echo "╔══════════════════════════════════════════════════════╗"
echo "║  SpoolHard Scale Release Build                      ║"
...
echo "SPOOLHARD-PRODUCT=spoolscale" > "$DATA_DIR/.spoolhard-product"
...
# scale/firmware/partitions.csv — bootloader at 0x0, partition table at
