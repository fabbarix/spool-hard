...
export function BackupRestoreSection() {
  ...
  const downloadBackup = async () => {
    ...
    try {
      ...
      const name = m
        ? m[1]
        : `spoolhard-console-${new Date().toISOString().slice(0, 10)}-backup.json`;
  ...
  return (
    <SectionCard
      ...
      icon={<Save size={16} />}
      description="Download a single JSON file capturing every configuration namespace and on-disk database the console owns. Re-upload the same file (on this or a sibling device) to restore the full state. The device reboots automatically after a successful restore."
    >
      {/* ── Download ─────────────────────────────────────────── */}
